...
def directed_modularity(edge_index: torch.Tensor, labels: np.ndarray, num_nodes: int) -> float:
    """Directed modularity Q_dir for a hard partition.

    Q_dir = (1/m) sum_ij [A_ij - k_out_i k_in_j / m] * delta(c_i, c_j),
    matching the directed-modularity matrix B used in the composite loss
    (Eq. 8--9 of the manuscript). This is the quantity used by the
    validation-free K-selection score J(K) in Eq. (1).
    """
    if edge_index.numel() == 0:
        return 0.0
    edges = edge_index.detach().cpu().numpy()
    m = float(edges.shape[1])
    if m <= 0:
        return 0.0
    kout = np.zeros(num_nodes, dtype=np.float64)
    kin = np.zeros(num_nodes, dtype=np.float64)
    for src, dst in edges.T:
        kout[int(src)] += 1.0
        kin[int(dst)] += 1.0
    # Intra-cluster observed edge mass.
    same = 0.0
    for src, dst in edges.T:
        if labels[int(src)] == labels[int(dst)]:
            same += 1.0
    q = same / m
    # Expected intra-cluster mass under the directed configuration model.
    expected = 0.0
    for c in np.unique(labels):
        members = np.where(labels == c)[0]
        expected += kout[members].sum() * kin[members].sum()
    q -= expected / (m * m)
    return float(q)
...
def estimate_module_count(data, max_k: Optional[int] = None, eta: float = 0.1, rho: float = 0.1, seed: int = 42) -> int:
    ...
    where Q_dir is the directed modularity defined in Eq. (1) and the
    directed-modularity matrix in Eq. (8). A lightweight KMeans over semantic
    embeddings is used for the sweep.
    """
    ...
    for k in range(2, upper + 1):
        ...
        q_dir = directed_modularity(data.edge_index, labels, n)
        ...
        score = q_dir + eta * sil - rho * imbalance
        if score > best_score:
            best_score = score
            best_k = k
    return int(best_k)
...
class DeepModuleTrainer:
    ...
    @staticmethod
    def paired_significance(deepmodule_runs: Iterable[float], baseline_runs: Iterable[float]) -> Dict[str, float]:
        """Paired Wilcoxon signed-rank test over repeated runs.

        Reproduces the significance claim in Section 4.4 (RQ1): the per-project
        MoJoFM improvement of DeepModule over the strongest non-DeepModule
        baseline is tested for significance across the repeated runs. Returns the
        statistic, p-value, and mean paired difference.
        """
        a = np.asarray(list(deepmodule_runs), dtype=float)
        b = np.asarray(list(baseline_runs), dtype=float)
        if a.shape != b.shape or a.size < 2:
            return {"statistic": float("nan"), "p_value": float("nan"), "mean_diff": float(np.mean(a - b)) if a.size else float("nan")}
        diff = a - b
        mean_diff = float(np.mean(diff))
        # Degenerate case: all differences identical -> deterministic separation.
        if np.allclose(diff, diff[0]):
            return {"statistic": 0.0, "p_value": 0.0 if diff[0] != 0 else 1.0, "mean_diff": mean_diff}
        try:
            from scipy.stats import wilcoxon
            stat, p = wilcoxon(a, b)
            return {"statistic": float(stat), "p_value": float(p), "mean_diff": mean_diff}
        except Exception:
            return {"statistic": float("nan"), "p_value": float("nan"), "mean_diff": mean_diff}
