...
class JavaProjectDataset:
    def __init__(self, root_dir):
        ...
        # Load CodeBERT (Lazy loading can be better, but initializing here for simplicity)
    ...
    def _get_embedding(self, file_path):
        """Helper to get embedding for a single file."""
        try:
            with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
                code = f.read()
            tokens = self.tokenizer.tokenize(code)[:510] # Truncate
    ...
    def build_graph(self):
        ...
        for i, path in enumerate(tqdm(self.file_paths)):
            ...
            # Naive Dependency Parsing (Replace with javalang AST parsing in production)
            try:
